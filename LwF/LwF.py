...
def parse_option():
    ...
    parser.add_argument('--fe_learning_rate', type=float, default=0.001, help='learning rate')
    ...
    args.save_path = '/data1/fh/ssd/pythonProject/VT-CIL/ArXiv/save/LwF/{}'.format(args.dataset)
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (feat, target) in enumerate(train_loader):
            ...
            if step == 0:
                ...
                loss_KD_same = touch_touch_loss_KD + vision_vision_loss_KD
                output_vision = output_vision[:, step * args.classes_per_step:]
                output_touch = output_touch[:, step * args.classes_per_step:]
                target = target % args.classes_per_step
                output_loss = criterion(output_vision, target) + criterion(output_touch, target)    
            loss_train = args.new_loss * output_loss + args.lambda_2 * loss_KD_same
            ...
            if idx % args.print_freq == 0:
                print('Epoch [{0}][{1}/{2}]\t'
                    ...
                    'output_loss {4:.6f}\t'
                    'loss_KD_same: {5:.6f}\t'
                    .format(epoch, idx, len(train_loader), losses_train, output_loss, args.lambda_2 * loss_KD_same))
                sys.stdout.flush()
