...
def parse_option():
    ...
    parser.add_argument('--exem_batch_size', type=int, default=4)
    parser.add_argument('--memory_size', type=int, default=300, help='number of examplars')
    ...
    args.save_path = '/data1/fh/ssd/pythonProject/VT-CIL/ArXiv/save/SS_IL/{}'.format(args.dataset)
...
def get_exemplar(args, step):
    ...
    loader = torch.utils.data.DataLoader(
            dataset, batch_size=args.exem_batch_size, shuffle=True,
...
def gen_step_exemplar_set(args, step, all_ground_path, exemplar_per_class):
    setup_seed(args.seed)
    all_ground_path_label = np.array([float(sample.split(',')[1]) for sample in all_ground_path])
    new_exemplars_path = []
    for class_id in range(step * args.classes_per_step, (step + 1) * args.classes_per_step):
        class_path = all_ground_path[all_ground_path_label == class_id].tolist()
        if len(class_path) < exemplar_per_class:
            print("exemplar size greater than samples.")
            sys.exit()
        filtered_exemplar = random.sample(class_path, exemplar_per_class)
        new_exemplars_path.append(filtered_exemplar)
    new_exemplars_path = np.array(new_exemplars_path)

    if step > 0:
        old_exemplar_name = 'exemplar.npy'
        path = os.path.join(args.save_path, old_exemplar_name)

        old_exemplar_class_path = np.load(path,allow_pickle=True)
        old_exemplar_path_filtered = old_exemplar_class_path[..., :exemplar_per_class]
        new_exemplars_path = np.concatenate((old_exemplar_path_filtered, new_exemplars_path), axis=0)
    return new_exemplars_path
...
def train(args, train_dataset, step):
    ...
    print("==> training...")
    class_separate = step * args.classes_per_step
    for epoch in range(args.epochs):
        ...
        if step > 0:
            iterator = zip(train_loader, cycle(exem_loader))
        else:
        ...
        for idx, (samples) in enumerate(iterator):
            ...
            if step == 0:
                ...
                if torch.cuda.device_count() > 1:
                    feat_vision, feat_touch = output_separete(model.module.forward(data))
                else:
                    feat_vision, feat_touch = output_separete(model(data))
                ...
                curr, prev = samples
                data, target = curr
                target = target % args.classes_per_step
                data_r, target_r = prev
                bz = data.shape[0]
                rbz = data_r.shape[0]
                data, data_r = data.cuda(non_blocking=True), data_r.cuda(non_blocking=True)
                data = torch.cat((data, data_r), dim=0)
                target, target_r = target.cuda(non_blocking=True), target_r.cuda(non_blocking=True)
                if torch.cuda.device_count() > 1:
                    feat_vision, feat_touch = output_separete(model.module.forward(data))
                else:
                    feat_vision, feat_touch = output_separete(model(data))
                output_vision = classifier(feat_vision)
                output_touch = classifier(feat_touch)
                curr_vision = output_vision[:bz, class_separate:]
                curr_touch = output_touch[:bz, class_separate:]
                curr_loss = criterion(curr_vision, target) + criterion(curr_touch, target)
                prev_vision = output_vision[bz:bz+rbz, :class_separate]
                prev_touch = output_touch[bz:bz+rbz, :class_separate]
                prev_loss = criterion(prev_vision, target_r) + criterion(prev_touch, target_r)
                output_loss = (curr_loss * bz + prev_loss * rbz) / (bz + rbz)
                with torch.no_grad():
                    if torch.cuda.device_count() > 1:
                        old_feat_vision, old_feat_touch = output_separete(old_model.module.forward(data))
                    else:
                        old_feat_vision, old_feat_touch = output_separete(old_model(data))
                    ...
                    old_vision = old_vision[:, :class_separate]
                    old_touch = old_touch[:, :class_separate]
                touch_touch_loss_KD = dis_loss(args, step, old_touch, output_touch, T)
                vision_vision_loss_KD = dis_loss(args, step, old_vision, output_vision, T)
                loss_KD_same = touch_touch_loss_KD.sum() + vision_vision_loss_KD.sum()
            loss_train = args.lambda_1 * output_loss + args.lambda_2 * loss_KD_same
            losses_train.update(loss_train.item(), data.size(0))
            loss_train.backward()
    ...
    if step != args.steps - 1:
        print('sampling exemplar...')
        examplar_per_class = args.memory_size // (args.classes_per_step * (step + 1))
        new_exemplar_class_path = gen_step_exemplar_set(args, step, train_dataset._dataset(), examplar_per_class)
        exemplar_name = 'exemplar.npy'
        exemplar_path = os.path.join(args.save_path, exemplar_name)
        np.save(exemplar_path, new_exemplar_class_path)
