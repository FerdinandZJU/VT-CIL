...
def parse_option():
    ...
    parser.add_argument('--memory_size', type=int, default=600, help='number of examplars')
    ...
    if args.dataset == "OFR":
        args.model_path = '/data1/fh/ssd/pythonProject/VT-CIL/ArXiv/save/pretrain/OFR/model_5/ckpt_epoch_240.pth'
    if args.data_folder is None:
        raise ValueError('data_folder is None.')


    args.save_path = '/data1/fh/ssd/pythonProject/VT-CIL/ArXiv/save/iCaRL/{}'.format(args.dataset)
...
def gen_exemplar_detail(args, model, step, dataset, examplar_per_class, modality=None):
    ...
    gen_exemplar_loader = torch.utils.data.DataLoader(dataset, batch_size=args.exem_batch_size, num_workers=args.num_workers,
                                      worker_init_fn=worker_init_fn if args.num_workers!=0 else None, pin_memory=True, drop_last=False, shuffle=False)
...
def loss_binary(old_output, new_output, target):
    old_output_sigmoid = torch.sigmoid(old_output)
    old_task_size = old_output_sigmoid.shape[1]
    target_binary = target.clone()
    target_binary[...,:old_task_size] = old_output_sigmoid
    return F.binary_cross_entropy_with_logits(new_output, target_binary)
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (feat, target) in enumerate(train_loader):
            ...
            if step == 0:
                ...
                loss_KD_same = loss_binary(old_vision, output_vision, target) + loss_binary(old_touch, output_touch, target)
                output_loss = args.lambda_2 * loss_KD_same
   
            loss_train = args.new_loss * output_loss
