...
class IncrementalLoader(Dataset):
    ...
    def _incremental_step(self, step):
        if self.mode == 'train':
            ...
            if not self.args.rehearsal_free:
                if step > 0:
                    exemplar_name = 'step_' + str(step - 1) + '_exemplar_class_labels_path.npy'
                    path = os.path.join(self.save_path, exemplar_name)
                    exemplars = np.load(path, allow_pickle=True)
                    for i in range(self.classes_per_step * step):
                        self.env.extend(exemplars[i])
        elif self.mode == 'test':
